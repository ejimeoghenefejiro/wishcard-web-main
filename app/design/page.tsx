...
import { ArrowLeft, Sparkles, Download, Share2, AlertTriangle, Save, Trash2, Facebook, MessageCircle } from "lucide-react";
...
function CreateCardContent() {
    ...
    const { user, loading: authLoading, incrementUsage, login } = useAuth();
    ...
    // For guests, we treat them as "free" but checking limits happens on Download/Share or we just let them generate 3
    // Validating guest limits is harder without fingerprinting. For now, unrestricted generation, gated download.
    const cardsRemaining = user ? Math.max(0, tierInfo.cardsPerMonth - cardsUsed) : 3; // Guests get "3" visually
    const isFreeTier = user ? tierInfo.watermark : true; // Guests are free tier
    ...
    useEffect(() => {
        ...
        if (shouldRestore === "true") {
            const savedState = localStorage.getItem("wishcard_guest_state");
            if (savedState) {
                try {
                    const parsed = JSON.parse(savedState);
                    setOccasion(parsed.occasion || "birthday");
                    setMessage(parsed.message || "");
                    setRecipientName(parsed.recipientName || "");
                    setSenderName(parsed.senderName || "");
                    setStyle(parsed.style || "Modern");
                    setFontStyle(parsed.fontStyle || "Elegant Script");
                    setTextColor(parsed.textColor || "White");
                    setTextPosition(parsed.textPosition || "Centered");
                    setTextEffect(parsed.textEffect || "Glow");
                    setGeneratedImage(parsed.generatedImage || null);

                    // Clear after restoring
                    localStorage.removeItem("wishcard_guest_state");

                    // Clean URL
                    router.replace("/design", { scroll: false });
                } catch (e) {
    ...
    const handleGenerate = async () => {
        ...
        // Guest generation is ALLOWED now.
        // Limits check: Only applies if logged in. Guests we let generate.

        if (user && user.cardsUsed >= tierInfo.cardsPerMonth) {
        ...
        try {
            ...
            // Increment usage only if logged in
            if (user) {
    ...
    const requireAuth = () => {
        if (!user) {
            // Save state
            const stateToSave = {
                occasion, message, recipientName, senderName, style, fontStyle,
                textColor, textPosition, textEffect, generatedImage
            };
            localStorage.setItem("wishcard_guest_state", JSON.stringify(stateToSave));

            // Redirect to login with restore param
            login("/design?restore=true");
            return false;
        }
        return true;
    };
    ...
    const handleSave = async () => {
        if (!generatedImage) return;
        if (!requireAuth()) return; // Auto-login if saving
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="glass-card rounded-[2rem] p-6 md:p-8 shadow-xl animate-slide-up">
                        ...
                        {/* Usage indicator for free tier - Show specially for guests */}
                        <div className={`mb-4 px-4 py-3 rounded-xl flex items-center justify-between text-sm font-medium ${cardsRemaining === 0 ? "bg-red-50 border border-red-200 text-red-700" :
                            ...
                            <span>{user ? (isFreeTier ? "🆓 Free Trial" : tierInfo.emoji + " " + tierInfo.name) : "🎁 Guest Access"}</span>
                            <span>{cardsRemaining === 0 ? "⚠️ Limit reached" : `${cardsRemaining} card${cardsRemaining === 1 ? "" : "s"} left`}</span>
                        </div>

                        {/* Generate Button */}
                        <button
                            onClick={handleGenerate}
                            disabled={generating || (!!user && cardsRemaining === 0)}
                        ...
                        >
                            {generating ? (
                                ...
                                <><Sparkles className="w-5 h-5" />Generate Card</>
                            )}
                        </button>
                        {user && cardsRemaining === 0 && (
                        ...
                        {!user && (
                            <p className="text-center text-xs text-gray-500 mt-3">Sign up not required to generate.</p>
                        )}
                    </div>

                    {/* Right: Preview */}
                    <div>
                        <div className="glass-card rounded-[2rem] p-6 md:p-8 shadow-xl h-fit lg:sticky lg:top-24 animate-slide-up" style={{ animationDelay: "0.2s" }}>
                            ...
                            {generatedImage ? (
                                <div className="space-y-6 animate-fade-in">
                                    <div className="relative aspect-square rounded-2xl overflow-hidden bg-gray-100 shadow-inner group">
                                        ...
                                        {/* Watermark for Free Tier or Guest */}
                                        {(isFreeTier || !user) && (
                                            <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center" style={{ background: "rgba(0,0,0,0)" }}>
                                                <div className="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-center py-2 px-3 flex items-center justify-center gap-2">
                                                    ...
                                                    {user ? (
                                                        <button onClick={() => router.push("/profile")} className="text-xs text-yellow-300 underline font-semibold hover:text-yellow-200">Remove</button>
                                                    ) : (
                                                        <button onClick={() => login("/profile")} className="text-xs text-yellow-300 underline font-semibold hover:text-yellow-200">Remove</button>
                                                    )}
                                                </div>
